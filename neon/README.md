...
| Path | Purpose |
| --- | --- |
| `neon/apostille-me-prod/terraform/` | pinned `kislerdm/neon` provider + lock file; project `apostille-me-prod` in `aws-us-east-2` with `prevent_destroy` |
...
State: **planned**. No apostille-me Neon organization or project has been provider-verified yet, so no ID in this repo is
...
- Migrations are authored in `apostille-me/apme-lib-core` and applied by `dpm`; this repository is not a migration authority.
